...
export default function MapScreen() {
  ...
  // Flip the live map to night when the theme changes (no reload).
  useEffect(() => { inject(`window.ZV && window.ZV.setNight(${isDark})`); }, [isDark]);
      ...
      <SafeAreaView edges={["top"]} style={st.topWrap} pointerEvents="box-none">
        <View style={st.searchRow} pointerEvents="box-none">
          <View style={st.search}>
            <Ionicons name="search" size={15} color={chrome} />
            <TextInput
            ...
            {q.length > 0 && (
              <Pressable onPress={() => { setQ(""); setSugs([]); }} hitSlop={8}>
                <Ionicons name="close-circle" size={16} color={c.slate} />
              </Pressable>
            )}
          </View>
        ...
        <View style={st.controls} pointerEvents="box-none">
          ...
          <Pressable onPress={() => setLayersOpen((o) => !o)} style={[st.hazBtn, activeLayers > 0 && st.hazBtnOn]}>
            <Ionicons name="layers-outline" size={14} color={activeLayers ? "#fff" : chrome} />
            <Text style={[st.hazBtnT, activeLayers > 0 && { color: "#fff" }]}>Hazards{activeLayers ? ` ${activeLayers}` : ""}</Text>
            <Ionicons name={layersOpen ? "chevron-up" : "chevron-down"} size={12} color={activeLayers ? "#fff" : c.slate} />
          </Pressable>
        </View>
        {layersOpen && (
      ...
      {busy && (
        <View style={st.loadWrap} pointerEvents="none">
          <View style={st.loadCard}>
            <View style={st.loadIcon}><ActivityIndicator color={c.goldDeep} /></View>
            <View>
...
function makeStyles(c: Palette) {
  const mapBg = c.isDark ? "#0a1022" : "#e8edf0";
  const frost = c.isDark ? "rgba(16,26,58,0.94)" : "rgba(255,255,255,0.96)";
  const frost2 = c.isDark ? "rgba(16,26,58,0.9)" : "rgba(255,255,255,0.92)";
  const divider = c.isDark ? c.line : "#eef1f5";
  return StyleSheet.create({
    root: { flex: 1, backgroundColor: mapBg },
    web: { flex: 1, backgroundColor: mapBg },
    ...
    sugs: {
      ...
      shadowColor: c.shadow, shadowOpacity: 0.18, shadowRadius: 16, shadowOffset: { width: 0, height: 8 }, elevation: 8,
    },
    sug: { flexDirection: "row", alignItems: "center", gap: 10, paddingHorizontal: 14, paddingVertical: 11, borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: c.line },
    sugMain: { fontSize: 13.5, fontWeight: "600", color: c.ink },
    sugSec: { fontSize: 11, color: c.slate, marginTop: 1 },

    controls: { alignSelf: "center", marginTop: 9, flexDirection: "row", alignItems: "center", gap: 8 },
    seg: {
      flexDirection: "row", backgroundColor: frost2, borderRadius: 100, padding: 3,
      shadowColor: c.shadow, shadowOpacity: 0.16, shadowRadius: 10, shadowOffset: { width: 0, height: 4 }, elevation: 4,
    },
    segB: { paddingHorizontal: 15, paddingVertical: 6, borderRadius: 100 },
    segOn: { backgroundColor: c.navy },
    segT: { fontSize: 11.5, fontWeight: "700", color: c.slate },
    segTOn: { color: "#fff" },
    ...
    hazBtn: { flexDirection: "row", alignItems: "center", gap: 5, backgroundColor: frost2, borderRadius: 100, paddingHorizontal: 12, paddingVertical: 7, shadowColor: c.shadow, shadowOpacity: 0.16, shadowRadius: 10, shadowOffset: { width: 0, height: 4 }, elevation: 4 },
    hazBtnOn: { backgroundColor: "#0f766e" },
    hazBtnT: { fontSize: 11.5, fontWeight: "700", color: c.isDark ? c.goldLite : c.navy },
    hazPanel: { alignSelf: "center", marginTop: 8, backgroundColor: c.card, borderRadius: 14, paddingVertical: 8, paddingHorizontal: 10, width: 212, borderWidth: 1, borderColor: c.line, shadowColor: c.shadow, shadowOpacity: 0.18, shadowRadius: 16, shadowOffset: { width: 0, height: 8 }, elevation: 8 },
    hazPanelH: { fontSize: 8.5, letterSpacing: 1.2, color: c.slate, fontWeight: "800", marginBottom: 4, paddingHorizontal: 4 },
    hazRow: { flexDirection: "row", alignItems: "center", gap: 9, paddingVertical: 7, paddingHorizontal: 4 },
    hazSw: { width: 18, height: 18, borderRadius: 6, borderWidth: 1.5, borderColor: c.line, alignItems: "center", justifyContent: "center", backgroundColor: c.card },
    ...
    sheet: {
      ...
      shadowColor: c.shadow, shadowOpacity: 0.3, shadowRadius: 24, shadowOffset: { width: 0, height: -10 }, elevation: 20,
    },
    ...
    cur: { fontFamily: SERIF, fontSize: 17, fontWeight: "600", color: c.isDark ? c.goldLite : c.navy, marginBottom: 3 },
    vnum: { fontFamily: SERIF, fontSize: 29, fontWeight: "700", color: c.ink, lineHeight: 30, letterSpacing: -0.5 },
    per: { fontSize: 10.5, color: c.slate, fontWeight: "600", marginBottom: 3 },

    hzWrap: { marginTop: 12, paddingTop: 12, borderTopWidth: 1, borderTopColor: divider },
    hloading: { fontSize: 11, color: c.slate, fontStyle: "italic" },

    nearWrap: { marginTop: 13, paddingTop: 12, borderTopWidth: 1, borderTopColor: divider, gap: 7 },
    nearH: { fontSize: 8.5, letterSpacing: 1.3, color: c.slate, fontWeight: "800" },
    nearRow: { flexDirection: "row", alignItems: "center", gap: 9 },
    nearVal: { fontFamily: SERIF, fontSize: 14, fontWeight: "700", color: c.ink, width: 104 },
    nearPer: { fontFamily: "System", fontSize: 9, color: c.slate, fontWeight: "600" },
    nearLine: { flex: 1, fontSize: 11.5, color: c.inkSoft, fontWeight: "500" },
